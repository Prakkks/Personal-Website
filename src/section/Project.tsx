    ...
    <div className="flex flex-col gap-5 relative sm:p-10 py-10 px-5 shadow-2xl shadow-black-200 ">
      ...
      <div className="flex justify-between items-center mt-7">
        {["previous", "next"].map((dir) => (
          <button
            key={dir}
            className={`w-10 h-10 p-3 rounded-full arrow-gradient text-white transition-all ${
              isSingle ? "opacity-50 cursor-not-allowed" : "cursor-pointer active:scale-95"
            }`}
          ...
          {projectCategories.map((project) => (
            <div
              className={`project-title ${
                activecategory === project ? "text-[#915eff] border-b-primary-purple" : ""
              }`}
              key={project}
              onClick={() => {
      ...
      <div className="grid lg:grid-cols-2 grid-cols-1 gap-5 w-full relative">
        {!isMobile ? (
          ...
          <div className="flex flex-col gap-5 relative sm:p-10 py-10 px-5 shadow-2xl shadow-black-200 ">
            ...
            <div className="flex flex-col gap-5 text-white-600 my-5">
              <p className="text-white font-semibold animatedText text-md sm:text-base">{currentProject.title}</p>
